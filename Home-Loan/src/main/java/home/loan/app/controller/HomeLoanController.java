...
import home.loan.app.config.exception.CustomerApprovedException;
import home.loan.app.config.exception.CustomerLandOrFinanceException;
import home.loan.app.config.exception.LoanNotApproved;
import home.loan.app.config.exception.LoginRequiredException;
import home.loan.app.config.exception.NotAccessibleException;
import home.loan.app.dto.CreateCustomerRequest;
...
import home.loan.app.entity.Emi;
import home.loan.app.entity.Loan;
import home.loan.app.entity.LoanTracker;
import home.loan.app.service.CustomerLoanService;
import home.loan.app.service.CustomerRegister;
import home.loan.app.service.CustomerService;
import home.loan.app.util.CustomerUtil;
import home.loan.app.util.LoanUtil;
...
public class HomeLoanController {
	...
	@Autowired
	private CustomerRegister cRegister;
	...
	private Logger logger = LoggerFactory.getLogger(HomeLoanController.class);

	// Only User
	@ResponseStatus(HttpStatus.CREATED)
	@PostMapping("/add")
	public CustomerDetailsDto addPersonalDetails(@RequestBody  CreateCustomerRequest requestData,
		...
		CustomerDetailsDto details = customerUtil.toDetails(customer);
		return details;
	}

	// Only Admin
	@GetMapping("/allcustomers")
	public List<CustomerDetailsDto> findall(HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (session.getAttribute("username") == null) {
			throw new LoginRequiredException("Please login first");
		}
		...
		List<CustomerDetailsDto> details = customerUtil.toDetails(customers);
		return details;
	}

	// Only Admin
	@GetMapping("/by/id/{id}")
	public CustomerDetailsDto findCustomerById(@PathVariable("id") int id, HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (session.getAttribute("username") == null) {
			throw new LoginRequiredException("Please login first");
		}
	...
	public LoanDetailsDto addLoan(@PathVariable("date") Date date, @PathVariable("applyamount") float amount,
			HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (session.getAttribute("username") == null) {
			throw new LoginRequiredException("Please login first");
		...
		LoanDetailsDto details = loanUtil.toDetails(loan);
		return details;
	}

	// Only FVO and Admin
	@GetMapping("/loan/updateFinanceDocuments/{customerid}/{applicationid}/{finance}")
	public LoanDetailsDto updateLoanFinance(@PathVariable("customerid") int id, @PathVariable("applicationid") int appid,
	...
	public LoanDetailsDto updateLoanLandDocuments(@PathVariable("customerid") int id,
	...
	public LoanDetailsDto updateAdminApproval(@PathVariable("customerid") int id, @PathVariable("applicationid") int appid,
	...
	public LoanDetailsDto deleteLoan(@PathVariable("appid") int appid, HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (session.getAttribute("username") == null) {
			throw new LoginRequiredException("Please login first");
		}
	...
	public List<LoanDetailsDto> getAllLoansByCustomerId(HttpServletRequest request) {
		HttpSession session = request.getSession();
		if (session.getAttribute("username") == null) {
			throw new LoginRequiredException("Please login first");
		}
		...
		List<LoanDetailsDto> details = loanUtil.toDetails(loans);
